...
export class IniMap {
  ...
  #comments: Comments = {
    ...
    setAtLine: (line: number, text: string): Comments => {
      ...
      const formatted =
        text.startsWith(mark) || text === "" ? text : `${mark} ${text}`;
      if (comment) {
  ...
  *#readTextLines(text: string): Generator<string> {
  ...
  toObject(): Record<string, unknown | Record<string, unknown>> {
    ...
    for (const { sec, map, id } of this.#sections.values()) {
      const section: Record<string, unknown> = {};
      Object.defineProperty(obj, id ? id : sec, {
        value: section,
        writable: true,
        enumerable: true,
        configurable: true,
      ...
      if (id) {
        Object.defineProperty(section, "__KEY__", {
          value: sec,
          writable: true,
          enumerable: true,
          configurable: true,
  ...
  toString(replacer?: ReplacerFunction): string {
    const replacerFunc: ReplacerFunction =
      ...
      ? this.#lines.filter((lineA, index, self) => {
          if (lineA.type === "value") {
            const lastIndex = self.findLastIndex((lineB) => {
              return (
                lineA.sec === (lineB as LineValue).sec &&
    ...
    return lines
      .map((line) => {
        switch (line.type) {
          ...
          case "value":
            return (
              line.key + assignment + replacerFunc(line.key, line.val, line.sec)
            );
        }
      })
  ...
  parse(text: string, reviver?: ReviverFunction): this {
    ...
    const reviverFunc: ReviverFunction =
    ...
    for (const line of this.#readTextLines(text)) {
      ...
      if (isComment(trimmed)) {
        ...
        this.#sections.set(
          currentSection.id ?? currentSection.sec,
          currentSection
        );
      } else {
        ...
        if (this.#formatting.pretty === undefined) {
          this.#formatting.pretty =
            leftHand.endsWith(" ") && rightHand.startsWith(" ");
        }
